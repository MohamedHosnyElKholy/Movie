...
import { fetchAllTrendingTv } from "../lib/headerslice";
...
export default function Trending() {
  ...
  const trending = useSelector(
    (state: RootState) => state.moviesSlice.tv
  ) as Movie[];

  useEffect(() => {
    dispatch(fetchAllTrendingTv());
  }, [dispatch]);
console.log(trending);
  return (
    <div className="container mt-[50px] mx-auto">
      <div className="flex justify-between items-center mb-3">
        <h2 className="text-[24px] text-[#F3F3E0]">Trending TV</h2>
        <Link
          href="/tv"
